...
async def create_user(user_id: int, first_name: str, last_name: str | None,
    ...
    """Create a new user record in the database and return the corresponding object.
    ...
    :param db_session: AsyncSession: The asynchronous database session used for the
     operation.

    :return: UserModel: An instance of the UserModel class representing the created
     user.
    """
    async with db_session as session:
        async with session.begin():
            user_obj: UserModel = \
                UserModel(user_id=user_id, first_name=first_name, last_name=last_name,
                          username=username,)
...
async def delete_user(user_id: int, db_session: AsyncSession) -> int | None:
    ...
    :return: int | None: The user_id of the deleted user, or None if the user doesn't
     exist.
    """
    async with db_session as session:
        async with session.begin():
