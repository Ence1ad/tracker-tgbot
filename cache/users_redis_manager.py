...
async def redis_sadd_user_id(user_id: int, redis_client: Redis) -> int:
    ...
    This function adds a user's ID to a Redis set for tracking users who have been sent a message.
...
async def redis_srem_user_id(user_id: int, redis_client: Redis) -> int:
    ...
    This function removes a user's ID from a Redis set of users who have been sent a message.
...
async def is_redis_sismember_user(user_id: int, redis_client: Redis) -> bool | None:
    ...
    This function checks if a user's ID exists in a Redis set of users who have been sent a message.

    :param user_id: int: The unique identifier of the user to check.
    :param redis_client: Redis: An instance of a Redis client.
    :return: bool | None: True if the user ID exists in the set, False if not, or None if there was an error.
    """
...
async def redis_smembers_users(redis_client: Redis) -> set | None:
    ...
    This function retrieves a set of all user IDs from a Redis set of users who have been sent a message.

    :param redis_client: Redis: An instance of a Redis client.
    :return: set | None: A set of user IDs if available, or None if the set is empty or an error occurred.
    """
