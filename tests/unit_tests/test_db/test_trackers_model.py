...
class TestTrackers:
    ...
    async def test_stop_tracker(
        ...
        with expectation:
            duration_returning = await select_tracker_duration(user_id, tracker_id, db_session=session)
    ...
    async def test_get_report(
        ...
        with expectation:
            res_fetchall = await select_weekly_trackers(user_id, db_session=session)
            assert res_fetchall != []
