...
from db.tracker.tracker_db_command import select_stopped_trackers, select_tracker_duration, delete_tracker
...
from tgbot.handlers.tracker_handlers.del_tracker import _get_right_tracker_markup
...
class TestActionsHandlers:
    ...
    async def test_get_tracker_options(
    ...
    @pytest.mark.parametrize(
            ...
            [
                (MAIN_USER_ID, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'select_category_4_tracker', does_not_raise()),
                (USER_ID_WITH_TRACKER_LIMIT, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'select_category_4_tracker', does_not_raise()),
                (USER_WITHOUT_TRACKER, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'select_category_4_tracker', does_not_raise()),
                (USER_WITHOUT_CATEGORIES, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'valid_data_text', does_not_raise()),
            ]
    )
    ...
    @pytest.mark.parametrize(
            "user_id, state, operation, answer_text, expectation",
            [
                (MAIN_USER_ID, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'new_tracker_text', does_not_raise()),
                (USER_ID_WITH_TRACKER_LIMIT, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'not_enough_data_text', does_not_raise()),
                (SECOND_USER_ID, TrackerState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER, 'new_tracker_text', does_not_raise()),
    ...
    async def test_create_tracker(
            ...
            i18n, buttons, db_session, state: FSMContext, operation, dispatcher: Dispatcher, bot,
    ):
        ...
        handler_result = data = action_name = None
        actions = await select_category_actions(user_id, category_id=category_id, db_session=db_session)
        print(actions)
        if actions:
        ...
        with expectation:
            ...
            if before_handler == after_handler:
                ...
                # await redis_delete_tracker(user_id, redis_cli)
                assert handler_result.reply_markup == await menu_inline_kb(await buttons.tracker_menu_stop(), i18n)
    ...
    async def test_select_launched_tracker(
        ...
        with expectation:
            ...
            if is_tracker:
                ...
                assert handler_result.text == started_tracker + i18n.get(answer_text)
    ...
    async def test_take_traker_4_delete(
        ...
        with expectation:
            ...
            if stopped_trackers:
                assert handler_result.text == i18n.get(answer_text)
                assert handler_result.reply_markup == await callback_factories_kb(stopped_trackers, enum_val=TrackerOperation.DEL)
            else:
                assert handler_result.text == i18n.get(answer_text)
    ...
    async def test_get_weekly_report(
        ...
        with expectation:
            ...
            if report and isinstance(handler_result, SendDocument):
                assert handler_result.document.filename == 'Weekly Report.xlsx'
        ...
        async with db_session as sess:
            async with sess.begin():
                res = await sess.execute(sa.select(TrackerModel.tracker_id).where(TrackerModel.user_id == user_id,
                                           cast(TrackerModel.track_end, DATE) == datetime.date.today()))
        tracker_id = res.scalar_one_or_none()
