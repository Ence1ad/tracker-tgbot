...
from tests.unit_tests.test_bot.utils import USER_ID, TEST_CHAT
from tgbot.handlers.categories_handlers.show_categories import _categories_list
...
class TestCategoriesHandlers:
    @pytest.mark.parametrize(
        ...
        [
            (USER_ID, AppButtons.categories_data.CREATE_CATEGORIES.name, 'category_limit_text', does_not_raise()),
            (USER_ID, AppButtons.categories_data.CREATE_CATEGORIES.name, 'new_category_text',
    ...
    async def test_prompt_new_category_handler(
        ...
        with expectation:
            ...
            if answer_text == 'category_limit_text':
                assert handler_returns.reply_markup == await menu_inline_kb(await buttons.category_limit_menu(), i18n)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # (100001, CategoryState.GET_NAME, None, 'added_new_category_text', pytest.raises(TypeError)),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (USER_ID, AppButtons.general_data.CATEGORIES_BTN.name, 'show_categories_text', does_not_raise()),
            (USER_ID, AppButtons.general_data.CATEGORIES_BTN.name, 'empty_categories_text',
             pytest.raises(AssertionError)),
            (USER_ID, AppButtons.general_data.CATEGORIES_BTN.name, '', pytest.raises(AssertionError)),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (USER_ID, AppButtons.categories_data.DELETE_CATEGORIES.name, 'select_category_text', CategoryOperation.DEL,
             does_not_raise()),
            (USER_ID, AppButtons.categories_data.CREATE_CATEGORIES.name, 'select_category_text', CategoryOperation.DEL,
             pytest.raises(AssertionError)),
            (USER_ID, AppButtons.categories_data.DELETE_CATEGORIES.name, 'select_category_text', CategoryOperation.UPD,
             pytest.raises(AssertionError)),
            (USER_ID, AppButtons.categories_data.DELETE_CATEGORIES.name, 'empty_categories_text', CategoryOperation.DEL,
             pytest.raises(AssertionError)),
            (123, AppButtons.categories_data.DELETE_CATEGORIES.name, 'empty_categories_text', CategoryOperation.DEL,
             does_not_raise()),
            (123, AppButtons.categories_data.DELETE_CATEGORIES.name, 'select_category_text', CategoryOperation.DEL,
             pytest.raises(AssertionError)),
        ]
    )
    async def test_get_del_categories(
            self, user_id: int, answer_text: str, data: str, expectation: does_not_raise,
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, cb_operation, expectation",
        [
            (USER_ID, AppButtons.categories_data.UPDATE_CATEGORIES.name, 'select_category_text', CategoryOperation.UPD,
             does_not_raise()),
            (USER_ID, AppButtons.categories_data.CREATE_CATEGORIES.name, 'select_category_text', CategoryOperation.UPD,
             pytest.raises(AssertionError)),
            (USER_ID, AppButtons.categories_data.UPDATE_CATEGORIES.name, 'select_category_text', CategoryOperation.DEL,
             pytest.raises(AssertionError)),
            (USER_ID, AppButtons.categories_data.UPDATE_CATEGORIES.name, 'empty_categories_text', CategoryOperation.UPD,
             pytest.raises(AssertionError)),
            (123, AppButtons.categories_data.UPDATE_CATEGORIES.name, 'empty_categories_text', CategoryOperation.UPD,
             does_not_raise()),
            (123, AppButtons.categories_data.UPDATE_CATEGORIES.name, 'select_category_text', CategoryOperation.UPD,
             pytest.raises(AssertionError)),
        ]
    )
    async def test_get_upd_categories(
            self, user_id: int, answer_text: str, data: str, expectation: does_not_raise,
            execute_callback_query_handler, buttons: AppButtons, i18n, cb_operation: Enum, db_session,
    ):
        handler_returns = await execute_callback_query_handler(user_id, data)
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, expectation",
        [
            (USER_ID, CategoryCD(operation=CategoryOperation.UPD, category_id=2, category_name='cat_name'),
             'new_category_text',  does_not_raise()),
            (USER_ID, CategoryCD(operation=CategoryOperation.UPD, category_id=2, category_name='cat_name'),
             'categories_is_fake_text', pytest.raises(AssertionError)),
            (USER_ID, CategoryCD(operation=CategoryOperation.READ, category_id=1, category_name='1'),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (100001, CategoryState.GET_NAME, None, 'new_cat_name_from_user', 'valid_data_text',
             pytest.raises(AssertionError)),
            (USER_ID, CategoryState.WAIT_CATEGORY_DATA, 'cat_name_from_user', 'new_cat_name_from_user',
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, expectation",
        [
            (USER_ID, CategoryCD(operation=CategoryOperation.DEL, category_id=1, category_name='1').pack(),
             'rm_category_text',  does_not_raise()),
            (USER_ID, CategoryCD(operation=CategoryOperation.DEL, category_id=1, category_name='1').pack(),
             'categories_is_fake_text', does_not_raise()),
            (USER_ID, CategoryCD(operation=CategoryOperation.READ, category_id=1, category_name='1').pack(),
             'categories_is_fake_text', pytest.raises(AssertionError)),
    ...
    async def test_del_category(
        ...
        # data = callback_class.pack()
        await redis_hmset_create_tracker(
                user_id=USER_ID, tracker_id='1', action_id=1, action_name='some_name',
