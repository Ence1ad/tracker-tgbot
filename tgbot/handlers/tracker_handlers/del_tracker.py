...
async def select_removing_tracker(call: CallbackQuery, db_session: async_sessionmaker[AsyncSession],
    ...
    if tracker:
        ...
        markup = await _get_right_tracker_menu_buttons(user_id, redis_client, buttons)
...
async def _get_right_tracker_menu_buttons(user_id: int, redis_client: Redis,
                                          buttons: AppButtons) -> InlineKeyboardMarkup:
    is_started_tracker: bool = await is_redis_hexists_tracker(user_id, redis_client)
    start_tracker_menu_buttons = await menu_inline_kb(await buttons.tracker_menu_start())
    stop_tracker_menu_buttons = await menu_inline_kb(await buttons.tracker_menu_stop())
