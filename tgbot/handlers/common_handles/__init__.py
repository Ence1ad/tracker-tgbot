...
def register_common_handlers() -> Router:
    ...
    from .settings_handler import command_settings_handler, language_settings, \
        set_user_lang
    ...
    router.message.register(command_cancel_handler,
                            Command(CommandName.CANCEL.name, ignore_case=True),
                            any_state)
    router.callback_query.register(
        callback_cancel_handler,
        F.data == AppButtons.general_btn_source.CANCEL_BTN.name, any_state
    )
    router.callback_query.register(
        exit_menu_handler, F.data == AppButtons.general_btn_source.EXIT_BTN.name
    )
    router.message.register(command_help_handler, F.text.startswith('/help'))
    router.message.register(command_settings_handler,
                            Command(CommandName.SETTINGS.name, ignore_case=True))
    router.callback_query.register(
        language_settings, F.data == AppButtons.settings_btn_source.LANGUAGE.name
    )
    router.callback_query.register(set_user_lang,
                                   (F.data.in_(AppButtons.settings_btn_list)))
