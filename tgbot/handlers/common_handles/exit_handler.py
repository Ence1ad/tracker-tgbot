...
async def exit_menu_handler(call: CallbackQuery, state: FSMContext, redis_client: Redis,
    ...
    :param call: CallbackQuery: Get the user id of the user who pressed a button
    :param state: FSMContext: Clear the state of the user
    :param redis_client: Redis: Access the redis database
    :param buttons: AppButtons: Access the buttons dictionary
    ...
    if await is_redis_hexists_tracker(user_id, redis_client):
        text = await started_tracker_text(user_id=user_id, redis_client=redis_client, i18n=i18n,
